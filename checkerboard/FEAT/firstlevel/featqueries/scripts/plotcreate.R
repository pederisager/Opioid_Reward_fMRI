...
ggplot(psdata, aes(x = time*2.208, y = data, col = drug)) +
  ...
  geom_point(size = 2) +
  geom_label(data = psdata[which.max(psdata$data),], aes(label = paste(subject,"-",session)), size = 10, show.legend = FALSE) +
  geom_label(data = psdata[which.min(psdata$data),], aes(label = paste(subject,"-",session)), size = 10, show.legend = FALSE) +
